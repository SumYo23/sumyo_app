...
class CookList(APIView):
    ...
    def get(self, request, num, like_id):
        Like
        # 사용자 정보
...
class LikeList(APIView):
    def get(self, request, num, like_id):
        ...
        like_object = Like.objects.get(id=like_id)
        user_number = like_object.user.user_number
        # cook_id = like_object.cook.id
        # 사용자 정보
        user, _ = User.objects.get_or_create(user_number=user_number)
        # 냉장고 재고
        user_ingredient = Refrigerator.objects.filter(user=user).values_list("ingredient", flat=True)
